...
# ===== Mamba 回归模型（支持自定义dropout） =====
class MambaRegressor(nn.Module):
    def __init__(self, input_dim, hidden_dim=64, dropout=0.2):
        super().__init__()
        self.dropout_rate = dropout  # 保存dropout率用于显示
...
print(f"Dropout rate: {model.dropout_rate} ({model.dropout_rate*100:.0f}%)")
...
# ========== 模型信息 ==========
print("\n========== Mamba+Dropout Model Information ==========")
total_params = sum(p.numel() for p in model.parameters())
trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
print(f"Total parameters: {total_params:,}")
print(f"Trainable parameters: {trainable_params:,}")
print(f"Network structure: Input(10) → Hidden(64) → MambaBlock × 2 (with Dropout) → Output(1)")
...
# ========== Dropout说明 ==========
print("\n========== Dropout Mechanism Description ==========")
print(f"This model uses Dropout regularization with rate {model.dropout_rate}:")
print("1. Dropout randomly drops {:.0f}% of neurons during training".format(model.dropout_rate*100))
print("2. Helps prevent overfitting by reducing co-adaptation")
print("3. Applied in each MambaBlock after linear transformations")
print("4. Disabled during evaluation/testing")
...
plt.title(f'Mamba+Dropout (rate={model.dropout_rate}) Training Convergence', fontsize=14, fontweight='bold')
plt.legend(fontsize=11)
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig('../figures/Mamba_Dropout_convergence_curve.png', dpi=300, bbox_inches='tight')
print("Convergence curve saved as: Mamba_Dropout_convergence_curve.png")
plt.show()

print("\n========== Mamba+Dropout Model Training Completed ==========")